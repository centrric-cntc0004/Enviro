...
const ReviewData = ({ navigation, data }) => {
    ...
    const RenderContacts = () => {
        ...
        const [isCheck35, setCheck35] = useState(reported_faults)
        const [isCheck36, setCheck36] = useState(corrected)
        const [isCheck37, setCheck37] = useState(no_action)
        const [isCheck38, setCheck38] = useState(scheduled_for_repair)
        ...
        <Text style={{paddingLeft:10, paddingTop: 10, color: textBlack, textAlign: 'justify' }}>{date_now}</Text>
