...
const EmployeeFiles = ({ navigation, route, team, dispatch }) => {
    ...
    const { sub_files } = team
    console.log("shjhjhjhj"+JSON.stringify(sub_files))
    ...
    const call_nextapi = (item) => {
        setName(item.name)
        setItem(item)
        dispatch(certificate_list2(item.id))
        setSection(2)
    ...
    const call_next = () => {
        // name = ""
        setName("")
        dispatch(certificate_list2(route.params.id))
        setItem("")
        setSection(1)
    }
                ...
                <View style={{ flex: 1, backgroundColor: lightGrey }}>
                    {section === 1 ? (
                        <View>
                            ...
                            <View style={{ height: 50, backgroundColor: textGrey, flexDirection: 'row' }}>


                                <Text style={{ alignSelf:'center', paddingLeft: 10, fontSize: 14 }}>{route.params.name}</Text>
