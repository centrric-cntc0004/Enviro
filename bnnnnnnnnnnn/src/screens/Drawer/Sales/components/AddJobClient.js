...
import { fetch_selected_payment, add_new_job_card, add_new_job_cardd } from './common/action'
...
const AddJobClient = ({ client_info, paymentData, types, addJob, selected_payment, client_list, navigation, route, sales_enviroWaste, dispatch }) => {
    ...
    if (client_info) {
        idd = client_info.id
        companyname = client_info.client_name
        subub = client_info.site_suburb
        postall = client_info.postal_code
        sitea = client_info.site_address
        buildn = client_info.building
        contn = client_info.company_contact_number
        conte = client_info.company_email
    }
    ...
    const onSelect = (index) => {
        ...
        //  setCompAddr(clientList.find(item => item.id=== parseInt( e.target.value)).)
    ...
    const [selectedData, setDropdownData] = useState(companyname)
    const [id, setid] = useState(idd)

    const [compName, setCompName] = useState('')
    const [compAddr, setCompAddr] = useState('')
    const [suburb, setSuburb] = useState(subub)
    const [postal, setPostal] = useState(postall)
    const [siteAddr, setSiteAddr] = useState('')
    const [buildName, setBuildName] = useState(buildn)
    const [contactName, setContactName] = useState(contn)
    const [contactNo, setContactNo] = useState('')
    const [contactEmail, setContactEmail] = useState(conte)
                        ...
                        <View style={styles.editableBoxStyle}>
                            {/* <ModalDropdown
                                ...
                                <View style={{ alignItems: 'center', flexDirection: 'row' }}>
                                    <View style={{ flex: 5 }}> */}
                            <Text
                                style={{ height: 40, marginTop: 5, color: mainGrey, fontSize: 14, paddingLeft: 5 }} >{selectedData}</Text>
                            {/* </View>
...
const mapStateProps = (state) => {
    const { selected_payment, types, client_info } = state.sales_enviroWaste

    const { client_list, } = state.client
    return { client_list, selected_payment, types, client_info }
}
...
export default connect(mapStateProps, mapDispatchToProps)(AddJobClient)
