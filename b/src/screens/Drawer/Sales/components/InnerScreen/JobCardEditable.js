...
const JobCardEditable = ({ sales_client_job_card, client_info, card_list, navigation, sales_enviroWaste, dispatch, job_card_lists }) => {
    let allFolders = ['9', '8']
    ...
    return (
        <View style={{ flex: 1, backgroundColor: mainWhite, }}>
            <View style={{ flex: 1, backgroundColor: mediumGrey, height: 50, marginTop: 20, flexDirection: 'row' }}>
                ...
                <TouchableOpacity onPress={() => navigation.navigate("AddJobClient", { clientinfo: client_info })}
                    style={{ justifyContent: 'center', alignItems: 'center', marginRight: 20 }}>
                    <Icon name="plus-circle" size={20} color={mainGrey} />
                </TouchableOpacity>
            </View>

            { sales_client_job_card.map((item, key2) => {
                let data2 = card_list
                let name = ""
                if (item.company_name != "") {
                    name = item.company_name
                ...
                return (
                    ...
                    <View >
                        {item.connected === false ? (
                            <TouchableOpacity onPress={() => getData(item)} style={{
...
const mapStateProps = (state) => {
    const { card_list, sales_client_job_card } = state.sales_enviroWaste
    return { card_list, sales_client_job_card }
}


export default connect(mapStateProps)(JobCardEditable)
