...
import { CLIENT_GET, CLIENT_SEARCH,CLIENT_TEMP_SERACH, CLIENT_EDIT, CLIENT_DELETE, CLIENT_PREVIOUS_SALES ,TEMP_CLIENT_LIST,CLIENT_ASSET_CRUD} from '../../../store/endpoint'
    ...
    return function (dispatch) {
        if (query) {
            ...
            Api('post', CLIENT_SEARCH +data+'/', formbody)
                .then(response => {
                    console.log("ijkfsdkf"+JSON.stringify(response))
...
export const search_tempclient_list = (query,data) => {
    return function (dispatch) {
        if (query) {
            let formbody = new FormData()
            formbody.append('key', query)
            console.log("CLIENT_TEMP_SERACH +data+'/'"+CLIENT_TEMP_SERACH +data+'/')
            Api('post', CLIENT_TEMP_SERACH +data+'/', formbody)
                .then(response => {
                    console.log("kjghjuj"+JSON.stringify(response))
                    dispatch({ type: 'TEMP_CLIENTS', tempClient: response.data })

                })
        } else {
            dispatch(fetch_temp_clients(data))
        }
    }
}
...
export const edit_client = (data, client_id, success, failed,type) => {
    ...
    return function (dispatch) {
        ...
        Api('patch-forms1', edit_url, data,success,failed)
            .then(response => {
                console.log("response from client"+(response.status))

               
                if(response.status===400){
                    console.log("400 staus"+(JSON.stringify(response)))

                    failed(response)
                }else if(response.status===200){
                    console.log("200 staus"+(JSON.stringify(response)))
