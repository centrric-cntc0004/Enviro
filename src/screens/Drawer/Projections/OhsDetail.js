...
import { View, Text, Image, TextInput, ActivityIndicator, TouchableOpacity, Alert, Linking } from 'react-native'
...
let formatted_date = ""
import { edit_news, delete_notify, delete_news, read_news } from './common/actions'
...
const OhsDetails = ({ navigation, route, dispatch }) => {
    ...
    var readstatu = route.params.items.user_read_status


    const [readstatuss, setreadstatus] = useState(readstatu)
    console.log("kjk" + readstatuss)
    ...
    const call_readapi = () => {
        ...
        dispatch(read_news(1, route.params.items.id))
        ...
        <View style={{ flex: 1, backgroundColor: mainWhite }}>
            ...
            <View style={{
                ...
                {route.params.items.file_attachment !== null ? (
                    <View style={{ flex: 0.3, flexDirection: 'row', marginRight: 10, marginBottom: 10 }}>
                    ...
                    <>
                        {button_enable === true ? (
                            <TouchableOpacity onPress={() => call_editapi()}
                                style={{ marginLeft: 10, justifyContent: 'center', alignItems: 'center', width: 100, height: 35, borderWidth: 0.3, borderColor: mainGrey, borderRadius: 40 }}>
                                {loader ? (
                                ...
                                <View
                                    style={{ opacity: 0.6, marginLeft: 10, justifyContent: 'center', alignItems: 'center', width: 100, height: 35, borderWidth: 0.3, borderColor: mainGrey, borderRadius: 40 }}>
