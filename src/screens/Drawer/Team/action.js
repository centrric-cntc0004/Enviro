...
import {
    ...
    TEAM_ALL_FOLDERS_LIST1, TEAM_ALL_FOLDERS_LIST_SITE
...
export const edit_employee = (data, employee_id, success, failed) => {
    return function (dispatch) {
        ...
        console.log("edit_url" + JSON.stringify(edit_url))

        console.log("edit_url" + JSON.stringify(data))
...
export const search_employees = (keyword,type) => {
    return function (dispatch) {
        if (keyword) {
            let formbody = new FormData()
            formbody.append('key', keyword)
            Api('post-form', TEAM_SEARCH , formbody)
                .then(response => {
                    dispatch({ type: 'TEAM_FETCHING_SUCCESS', teams: response })
                })
        } else {
            dispatch( team_list_term1())

            dispatch(team_list_current())
        }
...
export const certificate_lists = (id) => {

    return function (dispatch) {

        // const req_url = TEAM_CERTIFICATE + id + '/'
        let a = TEAM_ALL_FOLDERS_LIST_SITE + id + '/' + '1/'
        console.log("kjhkjhkjhjk" + a)
...
export const certificate_list3 = (id, folderid) => {

    return function (dispatch) {

        // const req_url = TEAM_CERTIFICATE + id + '/'
        let a = TEAM_ALL_FOLDERS_LIST_SITE + id + '/' + folderid + '/'
        console.log("kjhkjhkjhjk" + a)
