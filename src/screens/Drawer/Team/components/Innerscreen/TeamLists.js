...
import { View, Text, FlatList, TouchableOpacity, Dimensions, Image, Alert,TextInput,ActivityIndicator } from 'react-native'
...
import { tab_employee,search_employees, team_list_current1,team_list_current, team_list_term1, select_employee, certificate_list, team_list, all_vehicle_folders, designation_list } from '../../action'
let data = []
import NoDataContent from '../../../../../common/NoDataContent'
const TeamLists = ({isLoading, get_Data, tab, getTeamCurr,serach_employee, getTeamTerm, designation_list, team_list_api, all_folders_api, navigation, team, select_employee_data, dispatch, get_employee, get_certificate }) => {

    let width = Dimensions.get('window').width / 3.35
    width = width;
    let dummyData = ["Current Employee", "Terminated Employees"]
    const [selected, setSelected] = useState(tab)
    const [selected1, setSelected1] = useState(dummyData[0])
    ...
    const serach_employeesdata=(text,tab)=>{
        if(text!==""){
            serach_employee(text,tab)
        }
        if(text===""){
            if (tab === "Current Employee") {
                getTeamCurr()
            } else {
                getTeamTerm()
            }
    ...
    return (
        <View style={{ flex: 1, backgroundColor: mainWhite, marginTop: '1%',paddingHorizontal:10 }}>
            ...
            <View style={{  flexDirection: 'row', marginTop: '5%', paddingLeft: 25, borderRadius: 20, height: 42, borderWidth: 0.3, backgroundColor: mainWhite, borderColor: darkGrey }}>
                <TextInput
            ...
            {team_list.length === 0 ? (
                <View style={{ flex: 1, justifyContent: 'center', alignContent: 'center',marginTop:'40%' }}>
                   <NoDataContent/>
                    ...
                    <FlatList
                        style={{ backgroundColor: lightGrey,paddingBottom:60,marginTop:'3%' }}
                        data={team_list}
...
const mapDispatchToProps = (dispatch) => {
    return {
        ...
        getTeamCurr: () => dispatch(team_list_current()),
        ...
        serach_employee:(key,type)=>dispatch(search_employees(key,type))
