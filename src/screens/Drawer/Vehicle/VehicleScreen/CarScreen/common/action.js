...
export const fetch_truck_list = (page, type) => {
    return function (dispatch) {
        ...
        let get_url = VEHICLE_GET + type + '/' + 'car' + '/all/' + page + '/'

        console.log(get_url)
        Api('get', get_url)
            .then(response => {
                if (response.length > 0) {
                    console.log("kjsgdjsjcudzs" + JSON.stringify(response))
...
export const fetch_all_truck_list = (type) => {
    return function (dispatch) {
        let get_url = VEHICLE_GET + type + '/car/list/'
...
export const fetch_maintanace_list = (page, type) => {
    ...
    return function (dispatch) {
        dispatch({ type: 'TRUCK_FETCHING' })
        let get_url = VEHICLE_GET + type + '/car/' + 'maintenance/reports/' + page + '/'
...
export const __edit_maintanace = (type,maintanace_id, data, success, failed, page_no) => {
    return function (dispatch) {
        let edit_url = VEHICLE_GET + type+'/car/maintenance/report/edit/'+ maintanace_id+'/'
...
export const __create_maintanace = (type,data, success, failed) => {
    return function (dispatch) {
        ...
        Api('post-form', VEHICLE_CAR_MAINTANACE_ADD, data, success_res, failed)
            .then(response => {
            })
    }
}
...
export const __delete_maintanace = (type, maintanace_id, success, failed, page_no) => {
    return function (dispatch) {
        ...
        let delete_url = VEHICLE_GET + type + '/car/deleteFuelExpense/' + maintanace_id + '/'
...
export const fetch_fuel_list = (page, type) => {
    ...
    return function (dispatch) {
        dispatch({ type: 'TRUCK_FETCHING' })
        let get_url = VEHICLE_GET + type + '/car/getFuelExpenses/' + page + '/'

        Api('get', get_url)
            .then(response => {
                // console.log(response)
                if (response.length > 0) {
                    dispatch({ type: 'CAR_FUEL_LIST', fuellist: response, page: page })
                }
            })
    }
}
export const fuel_list_pagination = (page) => {
    ...
    return function (dispatch) {
        ...
        let get_url = VEHICLE_GET + 'car/' + page + '/'
        Api('get', get_url)
            .then(response => {
                if (response.length > 0) {
...
export const __edit_fuel = (type, maintanace_id, data, success, failed, page_no) => {
    return function (dispatch) {
        let edit_url = VEHICLE_GET + type + '/car/editFuelExpense/' + maintanace_id + '/'
...
export const __create_preinspection = (id, type, data, success, failed) => {
    return function (dispatch) {
        const success_res = () => {
            success()
        ...
        Api('post-form1', VEHICLE_TRUCK_PREINSPECTION+'car/', data, success_res, failed)
            .then(response => {
...
export const fetch_preinspection_list = (page, type) => {
    ...
    return function (dispatch) {
        ...
        let get_url = VEHICLE_TRUCK_PREINSPECTION + type + '/car/pre-inspection/' + page + '/'
        ...
        Api('get', get_url)
            .then(response => {
                ...
                if (response.length > 0) {
                    ...
                    dispatch({ type: 'CAR_PREINSPECTION_LIST', preinspect: response })
