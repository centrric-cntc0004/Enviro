...
const FeetListC = ({ navigation, intranet_enviro, dispatch ,route}) => {
    ...
    return (
        <ScrollView style={{ backgroundColor: lightGrey }}
            ...
            <View style={{ flex: 1, paddingHorizontal: 10, backgroundColor: lightGrey, marginBottom: 40 }}>
                ...
                {`${VEHICLE_FOLDER_LIST}${"car/"}${route.params.items.id}/`}
               FOLDEREDIT={VEHICLE_FOLDER_RENAME} FOLDERADD={OHS_AND_S_FILE_ADD} FOLDERCREATE={VEHICLE_FOLDER_CREATE} 
               FOLDERDELETE={VEHICLE_FOLDER_DELETE} SELETPATH={"vehicle_car"} SELECTID={route.params.items.id} />
...
export default connect(mapStateProps)(FeetListC)
