...
import { View, StyleSheet, Text, TextInput, TouchableOpacity, ScrollView, ActivityIndicator, Alert,KeyboardAvoidingView } from 'react-native'
import { mainWhite, darkGrey, mainGrey, lightGrey, mainBlue, textlightgrey } from '../../../../../common/Colors'
...
const EditFuelExpenses = ({ navigation, route ,all_truck_list,dispatch,companytypes}) => {
    ...
    const RenderContacts = () => {
        ...
        console.log("registration"+JSON.stringify(route.params.vehicleList))
        const [selectedData, setSelectedData] = useState(registration)
        const [indexData, setIndexData] = useState(ve)
        ...
        const selectData = (index) => {
            ...
            setIndexData(route.params.vehicleList[index].id)
        }

        let edit_btn_status = false

        if(ve !== indexData || date !== datenew ||  time !== timenew || truckRego !== truckRegonew 
            ...
            <ScrollView style={{ flex: 1 }}>
                <View style={{
                    ...
                    <View style={{
                        ...
                        <View style={{ flex: 1, flexDirection: 'row', }}>
                            <View style={styles.staticBoxStyle}>
                                <Text style={styles.textStyle} >Forklift Rego</Text>
                            </View>
                            <View style={{ flex: 0.1 }} />
                            <View style={styles.editableBoxStyle}>
                                <Text style={{ alignSelf: 'center' }}>:</Text>
...
const mapStateProps = (state) => {
    const { isLoading,all_truck_list,companytypes } = state.vehicle__truck
    return { isLoading,all_truck_list,companytypes }
}


export default connect(mapStateProps)(EditFuelExpenses)
