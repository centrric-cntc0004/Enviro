...
const RoasterList = ({ navigation, schedule_enviro, select_client_data, dispatch }) => {
    ...
    const RenderContacts = ({ navigation }) => {
        const { schedule_list, complete_schedule_list ,type} = schedule_enviro
        ...
        const call_weekapi = () => {
            ...
            dispatch(daywise_schedule_list(date2, formatted_date,type))
                                ...
                                <View style={{ flex: 1.8, flexDirection: 'column', backgroundColor: mainWhite }}>
                                    <View style={{ flexDirection: 'column', marginTop: 10 }}>
                                        <Text numberOfLines={1}
                                            style={{ paddingLeft: 10, color: mainGrey, textAlign: 'justify', fontSize: 14 }}>{item.client.client_name}</Text>
