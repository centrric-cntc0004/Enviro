...
const UpdateJobCard = ({ job_full, paymentData, types, addJob1, selected_payment, client_list, navigation, route, sales_enviroWaste, dispatch }) => {
    ...
    const RenderContacts = () => {
        ...
        // let subb = ""
        // if (job_full) {
            console.log("jasdjsahfj"+JSON.stringify(route.params.data))
        ...
        let pitdistance = route.params.data.pit_distacnce_from_truck_location
        ...
        let accre = job_full.access_restriction
        let acch = route.params.data.access_height
        let list = route.params.data.service_list
        let ppes = route.params.data.specific_ppe_reqired
        let ind=route.params.data.type_of_induction
        ...
        const [optionData, setOption] = useState(keyr)
        ...
        const [ppe, setppe] = useState(ppes)
        const [dateup, setDateUp] = useState(formatted_date)
        ...
        const [typeOfInd, setTypeOfInd] = useState(ind)
        ...
        const addJobCard = () => {
            ...
            let dateStr = new Date(dateup)

            let postData = {
                'tab_type'              :   route.params.data.tab_type,
                "client"                :   route.params.data.client,
                "service_list"          :   serviceDet,
                "payment_details"       :   selected_payment.name,
                "company_name"          :   route.params.data.company_name,
                "company_address"       :   route.params.data.company_address,
                "suburb"                :   route.params.data.suburb,
                "postal_code"           :   route.params.data.postal_code,
                "job_site_address"      :   route.params.data.job_site_address,
                "building_name"         :   route.params.data.building_name,
                "contact_email_address" :   route.params.data.contact_email_address,
                "contact_number"        :   route.params.data.contact_number,
                "job_site_contact_name" :   route.params.data.job_site_contact_name,
                "access_restriction"    :   access,
                "tc_required"           :   tcreq,
                "waste_data_form"       :   datareq,
                "access_height"         :   accessHeight,
                "key_required"          :   optionData,
                "security_required"     :   security,
                "induction_required"    :   induction,
                "contact_name"          :   contName,
                "phone_number"          :   contPh,
                "type_of_induction"     :   typeOfInd,
                "pit_distacnce_from_truck_location" :   pitLoc,
                "water_tap_location"    :   waterTap,
                "gumy_required"         :   gumyreq,
                "confined_space"        :   confined,
                "number_of_trucks_required" :   noTruck,
                "specific_ppe_reqired"  :   ppe,
                ...
                console.log("jhjh"+JSON.stringify(postData))
            const success = () => {
                setLoader(false)
...
null
                ...
                <View style={{ flex: 1, flexDirection: 'row', }}>
                    ...
                    {options.map((item, key) => {
                        console.log("tcreq"+tcreq)
                        console.log("item"+JSON.stringify(item))

                        return (

                            <TouchableOpacity onPress={() => settcreq(item.option)}
                                style={{ marginTop: 18, marginRight: '8%', flex: 0.5, backgroundColor: mainWhite, flexDirection: 'row' }}>
                                <Text style={{ color: textBlack }}>{item.option.toString()}</Text>
                ...
                <View style={{ flex: 1, flexDirection: 'row', }}>
                    ...
                    {options.map((item, key) => {
                        return (
                            ...
                            <TouchableOpacity onPress={() => setdatareq(item.option)}
                                style={{ marginTop: 18, marginRight: '8%', flex: 0.5, backgroundColor: mainWhite, flexDirection: 'row' }}>
                                <Text style={{ color: textBlack }}>{item.option.toString()}</Text>
                ...
                <View style={{ flex: 1, flexDirection: 'row', }}>
                    ...
                    {options.map((item, key) => {
                        return (
                            ...
                            <TouchableOpacity onPress={() => setOption(item.option)}
                                style={{ marginTop: 18, marginRight: '8%', flex: 0.5, backgroundColor: mainWhite, flexDirection: 'row' }}>
                                <Text style={{ color: textBlack }}>{item.option.toString()}</Text>
                                ...
                                {item.option === optionData ? (
                                    <View style={{
                                        ...
                                        {/* <Icon  
                                        name="check" color={'#000'} size={15}></Icon> */}
                ...
                <View style={{ flex: 1, flexDirection: 'row', }}>
                    ...
                    {options.map((item, key) => {
                        return (
                            ...
                            <TouchableOpacity onPress={() => setsecurity(item.option)}
                                style={{ marginTop: 18, marginRight: '8%', flex: 0.5, backgroundColor: mainWhite, flexDirection: 'row' }}>
                                <Text style={{ color: textBlack }}>{item.option.toString()}</Text>

                                {item.option=== security? (
                                    <View style={{
                                        height: 18,
                                        width: 18,
                                        marginLeft: 5,
                ...
                <View style={{ flex: 1, flexDirection: 'row', }}>
                    ...
                    {options.map((item, key) => {
                        return (
                            <TouchableOpacity onPress={() => setinduction(item.option)}
                                style={{ marginTop: 18, marginRight: '8%', flex: 0.5, backgroundColor: mainWhite, flexDirection: 'row' }}>
                                <Text style={{ color: textBlack }}>{item.option.toString()}</Text>

                                {item.option === induction ? (
                                    <View style={{
                                        height: 18,
                                        width: 18,
                                        marginLeft: 5,
                ...
                <View style={{ flex: 1, flexDirection: 'row', }}>
                    ...
                    {options.map((item, key) => {
                        return (
                            <TouchableOpacity onPress={() => setgumyreq(item.option)}
                                style={{ marginTop: 18, marginRight: '8%', flex: 0.5, backgroundColor: mainWhite, flexDirection: 'row' }}>
                                <Text style={{ color: textBlack }}>{item.option.toString()}</Text>
                ...
                <View style={{ flex: 1, flexDirection: 'row', }}>
                    ...
                    {options.map((item, key) => {
                        return (
                            ...
                            <TouchableOpacity onPress={() => setconfined(item.option)}
                                style={{ marginTop: 18, marginRight: '8%', flex: 0.5, backgroundColor: mainWhite, flexDirection: 'row' }}>
                                <Text style={{ color: textBlack }}>{item.option.toString()}</Text>

                                {item.option === confined ? (
                                    <View style={{
                                        height: 18,
                                        width: 18,
                                        marginLeft: 5,
                ...
                <View style={{ flex: 1, flexDirection: 'row', }}>
                    ...
                    {options.map((item, key) => {
                        return (
                            <TouchableOpacity onPress={() => setppe(item.option)}
                                style={{ marginTop: 18, marginRight: '8%', flex: 0.5, backgroundColor: mainWhite, flexDirection: 'row' }}>
                                <Text style={{ color: textBlack }}>{item.option.toString()}</Text>
                ...
                <View style={{ flex: 1, flexDirection: 'row', }}>
                    ...
                    <DatePicker
                        ...
                        onDateChange={(date) => { setDateUp(date) }}
